...
local ServerConfig = {
    ...
    webhooksTitle = "Ynov Life Test",
    webhooksColor = 2061822,
	webhooksColor2 = 16776960,
}
...
AddEventHandler('esx_society:withdrawMoney', function(society, amount)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
		if amount > 0 and account.money >= amount then
			...
			ServerToDiscord(ServerConfig.webhooksTitle, '[Argent retiré] __'  ..xPlayer.getName().. '__ vient de retirer `'..amount..'$` de la société `'..society.name..'`', ServerConfig.webhooksColor2)
		else
...
AddEventHandler('esx_society:depositMoney', function(society, amount)
	...
	if amount > 0 and xPlayer.getMoney() >= amount then
		...
		ServerToDiscord(ServerConfig.webhooksTitle, '[Argent déposé] __'  ..xPlayer.getName().. '__ vient de déposer `'..amount..'$` dans la société `'..society.name..'`', ServerConfig.webhooksColor)
	else
		TriggerClientEvent('esx:showNotification', xPlayer.source, _U('invalid_amount'))
	end
end)
