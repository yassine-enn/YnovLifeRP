  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
	  ...
	  if #currentContent >= getVehicleMaxCarry(GetVehicleClass(currentVehicle)) then
		...
		{
                        css   = 'coffre',
			title = "Etes-vous sur de vouloir prendre une arme à feu" .. data.current.label .. "?"
...
function OpenTrunkMenu()
		...
		table.insert(elements, {label = '[ déposer argent sale ]', value = 'add_dirty'})
...
Citizen.CreateThread(function ()
	...
	while true do
		Citizen.Wait(5)
		if IsControlJustReleased(0, 182) and not trunkIsOpen and not IsPedInAnyVehicle(GetPlayerPed(-1)) and GetVehicle() and IsDistanceOk() and not IsDoorLocked() then
			if currentVehicle ~= nil then
				local vehicleType = GetVehicleClass(currentVehicle)
